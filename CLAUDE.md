...
**THEN IT SEARCHES INSIDE THAT SITE, BY TITLE — and the sentence that used to
sit here was wrong in a way that cost her two sessions.** It said the step
searches inside the domain "the way stage one searches inside the shop". It is
not the same thing: stage one OPENS the shop's pages, and `site:hannibalbooks.be
Metamorphoses` is a general index with a hint in it — the exact pattern §4 spent
21 Sep removing, left standing one step further down.

**HER INSTRUCTION WAS TO OPEN THE PUBLISHER'S OWN SEARCH BOX. IT CANNOT BE
BUILT, and that is measured, not argued.**
`hannibalbooks.be/en/search?q=9789493416543` shows the right book in her
browser — her screenshot — and comes back to the page reader as a newsletter
form and a footer. **Their results are drawn by JavaScript after the page
loads**, which is §5's founding fact about this whole project met in a shop
rather than a museum. Tested at that exact address with that exact ISBN. It is
worth revisiting the day a publisher's box turns out to be readable, because
their box DOES match an ISBN and no index does.

**SO THE INDEX STAYS THE FINDER, because it is the half that worked.** Asked
...
number. It was one of three queries here and could only dilute the slate.
`publisherSiteQueries` sends the whole title and the title before its colon,
and nothing else.

**WHAT ACTUALLY LOST METAMORPHOSES WAS THE READING, and her diagnostic said so
in one line.** Every step ran. The four results held Hannibal's homepage, two
lists and the **Dutch** edition's page, and the model was asked to judge
*Metamorfosen – Ovidius en de kunsten* against a row reading *Metamorphoses:
Ovid and the Arts*. It called it a different book and answered nothing, with
the book's page in its hand. **It is one page in two languages** — put `/en/`
in front of the Dutch address and the title reads *Metamorphoses – Ovid and the
Arts*, with an NL/EN switch on it.

So the prompt now says outright that **a translated title is the same book**,
names that pair as its example, says to read the results **in English**, and
says to prefer an `/en/` address where both are shown.
...
letters. Fixtures C-052 to C-058a.

**I REPORTED THE SEARCH BOX AS UNABLE TO MATCH AN ISBN, AND IT WAS A BAD
TEST.** Asked whether their box could find the book by ISBN, I ran a `site:`
query against a general index and reported "no" as a fact about the box. She
settled it with one screenshot of the box itself.

**NOT TESTED FROM A SESSION: the reading.** The search half is verified live;
pressing her button is not something a session can do, and the fixtures say so
...
- Rejecting a page because its title is in another language. A Dutch
  publisher prints the same book in Dutch and English on ONE page with a
  language toggle; the model was handed it and said no.
- Searching *about* a website instead of going to it, one step below the
  ...
  **A ruling about where to look applies to every step that looks, not only the
  one that prompted it** — though going to their box turned out to be
  impossible here, their results being JavaScript.
